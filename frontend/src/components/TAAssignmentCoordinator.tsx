...
export default function TAAssignmentCoordinator({
  ...
  const [selectedCourse, setSelectedCourse] = useState<CourseAssignment | null>(
    null
  );
  ...
  const assignmentsByTA: TAAssignment[] = result
    ? Object.entries(result.workloads).map(([taName, load]) => ({
        ...
        courses: Object.entries(result.assignments)
          .filter(([courseCode, info]: any) => info.tas.includes(taName))
    ...
    <div className="space-y-6">
      ...
      >
        <DialogContent
          ...
          className="sm:max-w-lg p-0 max-h-[85vh] overflow-y-scroll overscroll-contain"
          style={{ scrollbarGutter: "stable" }}
        >
          <div className="sticky top-0 z-10 bg-background border-b px-6 py-6 pb-4">
            <DialogHeader>
